...
from app.superset_client import DASHBOARD_SLUGS, UnknownDashboardError

router = APIRouter(tags=["superset"])


@router.get("/superset/guest-token")
def guest_token(dashboard: str = Query(..., description=f"One of {sorted(DASHBOARD_SLUGS)}")):
    try:
        token, embedded_uuid = superset_client.fetch_guest_token(dashboard)
    ...
    return {"token": token, "dashboard_id": embedded_uuid}
